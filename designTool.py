...
def geometry(airplane):
    ...
    #cr_w = (2*b_w*np.tan(AR_w) - 4*xt_w)/(taper_w - 1)
...
def aerodynamics(airplane, Mach, altitude, CL, W0_guess,
    ...
    if Mach > 0.4:
        ...
        Mach_dd = 0.94/np.cos(sweep_w) - tc_avg/np.cos(sweep_w)**2 - CL/10/np.cos(sweep_w)**3
    ...
    if flap_type is not None:
        ...
        # flap_def - Max deflection from Torenbeek flap chart (Tab 7-2, page 253)
        if flap_type == 'plain':
            dclmax = 0.9
            Fflap = 0.0144
    ...
    if x_nlg is not None: # Check if we have a LG
        ...
        '''
        # Estimate the aircraft weight based on usual wing loadings (Raymer Tab 5.5)
        # Maybe this could be used to remove W0 dependency
...
def fuel_weight(W0_guess, airplane, range_cruise):
    ...
    # Cruise C - We need to keep W0 guess because the function uses MTOW
    ...
    LDmax = 0.5/np.sqrt((dragDict['CD0']+dragDict['CDwave'])*dragDict['K'])
...
def weight(W0_guess, T0_guess, airplane):
    ...
    while abs(delta) > 10:
        ...
        Wf, Mf_cruise = fuel_weight(W0_guess, airplane, range_cruise=range_cruise)
...
def balance(airplane):
    ...
    # Tank CG
    xcg_f, b_tank_b_w = tank_cg(airplane, fuel_frac=1.0)
...
def doc(airplane, CEF=6.0, plot=False):
    # This function computes Direct Operating Cost (DOC)
    # using Roskam's Part VIII methodology.
    ...
    # Estimate block fuel
    Wf, _ = fuel_weight(W0, airplane, range_cruise=Rbl)
...
def tank_cg(airplane, fuel_frac):
    '''
